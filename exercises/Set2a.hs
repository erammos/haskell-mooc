...
palindromify = palidromify'
palidromify' "" = ""
palidromify' s = if isPalindrome s
                     then s
                     else palidromify' (substring 1 (length s - 1) s)
------------------------------------------------------------------------------
...
eitherDiv x y | y == 0 =  Left (show x ++ "/0")
